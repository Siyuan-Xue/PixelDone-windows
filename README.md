...
当前工作树是 PixelDone for Windows 3.2.9 正式版。3.2.9 修复了从待办编辑弹窗发起删除时，删除确认框被编辑弹窗遮挡而无法操作的问题。

## 版本基线

- Android：配套正式版为 PixelDone 3.2.5（versionCode 86，Room v7）。
- Windows：当前正式版本为 3.2.9，安装身份与数据目录保持不变。
...
3.2.9 正式安装包使用 `PixelDone_3.2.9_x64-setup.exe`。正式默认目录是 `%LOCALAPPDATA%\PixelDone`，主程序名为 `PixelDone.exe`。
...
3.2.9 保留 3.2.8 的统一危险删除确认与 Trash 筛选能力，并确保从待办编辑弹窗打开的删除确认框始终位于编辑弹窗之上且可点击。左侧清单栏继续支持 200–720px 的持久化范围，同时动态为主工作区保留至少 440px。验证记录见 3.2.9 release evidence。
...
Supabase 3.2 Storage 策略和公共 schema 保持不变。3.2.9 是正式发布；发布清单仍明确保留六项跨设备云端场景作为已授权但尚未完成的验证项，不应将它们误报为已验证。
