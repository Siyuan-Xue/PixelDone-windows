...
正式 Release 必须同时具备 Authenticode 证书和 Tauri updater signature；缺少任一签名材料时 CI 会中止。RC 可以用于内部验证，但不得标记为正式发布。
...
# 同时需要可用的 Authenticode 证书；正式发布推荐交给 release-windows.yml。
