...
PixelDone for Windows 3.2.1 正式版源码。3.2.1 修复桌面端 Options、边栏、Dock、设置按钮、本地化优先级和 Windows 通知身份。

## 版本基线

- Android：当前正式版为 PixelDone 3.2.0，使用 Room v6。
- Windows：当前正式版本为 PixelDone for Windows 3.2.1；安装身份、数据目录与 3.1.x 保持不变。
...
当前正式版使用 `PixelDone_3.2.1_x64-setup.exe` 完成当前用户安装。正式默认目录是 `%LOCALAPPDATA%\PixelDone`，主程序名为 `PixelDone.exe`。路径中的 `test` 仅可能来自历史验收时显式指定的临时目录，不是正式安装器配置。
...
3.2.1 沿用 3.2.0 的同步协议，并统一 Options 命名、扩大可调边栏、复刻 Android Dock 图标、改用固定尺寸设置图标按钮、缩小完成控件并修复优先级本地化；验证记录见 3.2.1 parity evidence。
...
Supabase 3.2 Storage 策略、公共 schema 迁移及总验收已通过，不得修改 Supabase 托管的 Storage 表所有权。用户在知悉 parity 尚未达到 100% 后明确授权发布 3.2.1；双设备原图同步、密码全局退出与 Realtime 安装态回归仍保持未验证状态，不得表述为已通过。3.2.1 已修复并自动测试通知快捷方式目标校验，但真实安装后的通知展示仍需独立人工回归。
