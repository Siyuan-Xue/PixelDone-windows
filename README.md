...
当前工作树是 PixelDone for Windows 3.3.0 正式版。3.3.0 在可配置 Dock 中新增 Markdown 清单导出。

## 版本基线

- Android：配套正式版为 PixelDone 3.3.0（versionCode 90，Room v7）。
- Windows：当前正式版本为 3.3.0，安装身份与数据目录保持不变。
...
3.3.0 正式安装包使用 `PixelDone_3.3.0_x64-setup.exe`。正式默认目录是 `%LOCALAPPDATA%\PixelDone`，主程序名为 `PixelDone.exe`。
...
3.3.0 在可配置 Dock 中新增 Markdown 导出：简略复制包含标题和任务复选框，详细复制额外包含优先级、截止日期与重复规则；导出包含当前清单的全部任务并保持当前排序。左侧清单栏继续支持 200–720px 的持久化范围，同时动态为主工作区保留至少 440px。验证记录见 3.3.0 release evidence。
...
- 已登录时点击“修改密码”会打开同规格弹窗；修改成功后当前设备退出登录，成功提示不会使用顶部红色告警。
...
- 普通 revision 竞态会静默刷新并自动重放一次，不再直接显示红色“状态过期”；网络同步失败会按 1、2、4、8、16、30 秒退避持续重试，新本地修改或 Realtime 事件会重置退避。
- Sync 行按网络重试、认证过期、服务端版本、本地存储、远端数据和未知错误显示本地化说明；顶部红色告警只保留初始化、数据库、不可恢复的 Windows 操作和图片读取错误。
...
Supabase 3.2 Storage 策略和公共 schema 保持不变。3.3.0 是正式发布；发布清单仍明确保留六项跨设备云端场景作为已授权但尚未完成的验证项，不应将它们误报为已验证。
