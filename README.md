...
PixelDone for Windows 3.1.3 候选版。当前公开正式版仍为 3.1.2；功能与领域语义以 Android 3.1.1 为权威，桌面端复用 Android 的暖色像素设计语言，并采用适合键鼠操作的清单侧栏与任务主工作区两栏布局。

## 版本基线

- Android：PixelDone 3.1.1，Room v5；Android UI 与共享业务语义继续作为 Windows 对齐基准。
- Windows：当前正式版本为 PixelDone for Windows 3.1.2；安装/更新版本与 Android 业务协议版本独立演进。
- Windows：当前源码版本为 3.1.3 release candidate，尚未创建 Git tag 或公开 Release。
...
正式版使用 `PixelDone_3.1.2_x64-setup.exe` 完成当前用户安装。正式默认目录是 `%LOCALAPPDATA%\PixelDone`，主程序名为 `PixelDone.exe`。路径中的 `test` 仅可能来自历史验收时显式指定的临时目录，不是正式安装器配置。
...
3.1.3 候选版已实现以下桌面结构；静态检查、生产构建、10/10 桌面 E2E、1000/1180/1440 响应式布局、深色模式、七语言/RTL 检查和无签名 NSIS 验证均已完成。
...
3.1.3 正式 Release 必须通过当前 `parity/pixeldone-3.1.3.yaml` Windows 目标清单的 100% required gate；该清单以 `parity/pixeldone-3.1.0.yaml` 固定的 Android 3.1.0 功能基线为来源，并叠加 Windows 3.1.3 的实现与验收状态。Supabase Storage 图片同步和 Android UI 不可达的 batch move 是明确排除项，不计为功能缺失。
