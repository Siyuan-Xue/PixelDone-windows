...
PixelDone for Windows 3.1.2。功能与领域语义以 Android 3.1.1 为权威，桌面端复用 Android 的暖色像素设计语言，并采用适合键鼠操作的清单侧栏与任务主工作区两栏布局。
...
3.1.2 已实现以下桌面结构；静态检查、生产构建、10/10 桌面 E2E、1000/1180/1440 响应式布局、深色模式、七语言/RTL 检查和无签名 NSIS 验证均已完成。
...
正式 Release 必须通过当前 `parity/pixeldone-3.1.2.yaml` Windows 目标清单的 100% required gate；该清单以 `parity/pixeldone-3.1.0.yaml` 固定的 Android 3.1.0 功能基线为来源，并叠加 Windows 3.1.2 的实现与验收状态。Supabase Storage 图片同步和 Android UI 不可达的 batch move 是明确排除项，不计为功能缺失。
