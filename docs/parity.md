...
> 本文件由 `bun run parity:report` 从 `parity/pixeldone-3.2.8.yaml` 生成，请勿手工维护状态。

Windows 目标：PixelDone Windows 3.2.8（formal_release）。
...
| IMAGE-LOCAL-001 | image | 单张本地图片添加、预览、替换、删除、缩放和平移 | 是 | in_progress | none |
