...
const fileName = 'PixelDone_3.2.0_x64-setup.exe';
...
writeFileSync(join(directory, 'latest.json'), `${JSON.stringify({
  version: '3.2.0',
  notes: 'PixelDone for Windows 3.2.0 adds verified password changes with global logout and private Supabase Storage image synchronization.',
  pub_date: new Date().toISOString(),
  platforms: {
    'windows-x86_64': {
      signature,
      url: `https://github.com/Siyuan-Xue/PixelDone-windows/releases/download/v3.2.0/${fileName}`
    }
  }
}, null, 2)}\n`, 'utf8');
