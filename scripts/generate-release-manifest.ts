...
const fileName = 'PixelDone_3.2.6_x64-setup.exe';
...
writeFileSync(join(directory, 'latest.json'), `${JSON.stringify({
  version: '3.2.6',
  notes: 'PixelDone for Windows 3.2.6 adds a focused password dialog and makes cloud synchronization recover automatically from transient races and network failures.',
  pub_date: new Date().toISOString(),
  platforms: {
    'windows-x86_64': {
      signature,
      url: `https://github.com/Siyuan-Xue/PixelDone-windows/releases/download/v3.2.6/${fileName}`
    }
  }
}, null, 2)}\n`, 'utf8');
