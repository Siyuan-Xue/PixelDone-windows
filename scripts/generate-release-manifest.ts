...
const fileName = 'PixelDone_3.1.3_x64-setup.exe';
...
writeFileSync(join(directory, 'latest.json'), `${JSON.stringify({
  version: '3.1.3',
  notes: 'PixelDone for Windows 3.1.3 refines the desktop shell, adds a persistent resizable sidebar, and hardens the Windows notification queue.',
  pub_date: new Date().toISOString(),
  platforms: {
    'windows-x86_64': {
      signature,
      url: `https://github.com/Siyuan-Xue/PixelDone-windows/releases/download/v3.1.3/${fileName}`
    }
  }
}, null, 2)}\n`, 'utf8');
