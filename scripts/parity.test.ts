...
describe('parity manifest', () => {
  test('targets Windows 3.2.6 while preserving the audited Android baseline', async () => {
    const manifest = await loadManifest();
    expect(manifest.windowsTarget).toEqual({
      product: 'PixelDone Windows',
      version: '3.2.6',
      stage: 'formal_release',
      evidence: ['parity/evidence/windows/candidate-3.2.6.md'],
  ...
  test('records the 3.2.6 installer as verified while cloud exceptions stay explicit', async () => {
    const manifest = await loadManifest();
    ...
    expect(installer?.evidence.windows).toContain('parity/evidence/windows/candidate-3.2.6.md');
