...
describe('parity manifest', () => {
  test('targets Windows 3.1.3 while preserving the Android 3.1.0 functional baseline', async () => {
    const manifest = await loadManifest();
    expect(manifest.windowsTarget).toEqual({
      product: 'PixelDone Windows',
      version: '3.1.3',
      stage: 'release_candidate',
      evidence: ['parity/evidence/windows/candidate-3.1.3.md']
    });
  ...
  test('accepts verified desktop and release-integrity rows for 3.1.3', async () => {
    const manifest = await loadManifest();
    for (const id of ['LIST-FIXED-001', 'TODO-CRUD-001', 'SETTINGS-DOCK-001']) {
      ...
      expect(row?.evidence.windows).toContain('parity/evidence/windows/candidate-3.1.3.md');
    }
    const installer = manifest.rows.find((candidate) => candidate.id === 'RELEASE-NSIS-001');
    expect(installer?.status).toBe('verified');
    expect(installer?.evidence.windows).toContain('parity/evidence/windows/candidate-3.1.3.md');
