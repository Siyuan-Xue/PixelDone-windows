...
describe('3.2.4 version contract', () => {
  test('keeps frontend, Tauri, Cargo, updater and E2E versions aligned', () => {
    ...
    const parityManifest = JSON.parse(
      readFileSync(join(root, 'parity', 'pixeldone-3.2.4.yaml'), 'utf8')
    );
    const updateE2e = readFileSync(join(root, 'e2e', 'specs', 'update.e2e.ts'), 'utf8');
    const candidateEvidence = readFileSync(
      join(root, 'parity', 'evidence', 'windows', 'candidate-3.2.4.md'),
      'utf8'
    );
    ...
    expect(tauriConfig.version).toBe('3.2.4');
    expect(cargoToml).toMatch(/^version = "3\.2\.4"$/m);
    expect(cargoLock).toMatch(/name = "pixeldone-windows"\r?\nversion = "3\.2\.4"/);
    expect(releaseManifest).toContain("version: '3.2.4'");
    expect(releaseManifest).toContain('PixelDone_3.2.4_x64-setup.exe');
    expect(previewClient).toContain("currentVersion: '3.2.4'");
    expect(releaseIntegrity).toContain('PixelDone_3.2.4_x64-setup.exe');
    expect(releaseWorkflow).toContain('- run: bun run test');
    expect(releaseWorkflow).toContain('PixelDone_3.2.4_x64-setup.exe.sha256');
    expect(parityManifest.baselineManifest).toBe('pixeldone-3.1.0.yaml');
    expect(parityManifest.windowsTarget.version).toBe('3.2.4');
    expect(parityManifest.windowsTarget.stage).toBe('formal_release');
    expect(updateE2e).toContain("currentVersion).toBe('3.2.4')");
    expect(candidateEvidence).toContain('# Windows 3.2.4 formal release evidence');
  });
