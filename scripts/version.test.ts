...
describe('3.2.8 version contract', () => {
  test('keeps frontend, Tauri, Cargo, updater and E2E versions aligned', () => {
    ...
    const parityManifest = JSON.parse(
      readFileSync(join(root, 'parity', 'pixeldone-3.2.8.yaml'), 'utf8')
    );
    const updateE2e = readFileSync(join(root, 'e2e', 'specs', 'update.e2e.ts'), 'utf8');
    const candidateEvidence = readFileSync(
      join(root, 'parity', 'evidence', 'windows', 'candidate-3.2.8.md'),
      'utf8'
    );
    ...
    expect(tauriConfig.version).toBe('3.2.8');
    expect(cargoToml).toMatch(/^version = "3\.2\.8"$/m);
    ...
    expect(previewClient).toContain("currentVersion: '3.2.8'");
    ...
    expect(parityManifest.windowsTarget.version).toBe('3.2.8');
    expect(parityManifest.windowsTarget.stage).toBe('formal_release');
    expect(updateE2e).toContain("currentVersion).toBe('3.2.8')");
    expect(candidateEvidence).toContain('# Windows 3.2.8 formal release evidence');
  });
